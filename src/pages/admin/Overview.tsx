...
export default function Overview() {
    ...
    // Local Project Data (for Gestor de Acervo dashboard)
    ...
    const fetchLocalStats = async () => {
        ...
        // 2. Count species in project (via especie_local pivot table)
        const { count: speciesCount } = await supabase
            .from('especie_local')
            .select('*', { count: 'exact', head: true })
            .eq('local_id', profile.local_id);

        // 3. Count team members (users linked to this project)
        const { count: teamCount } = await supabase
            .from('profiles')
            .select('*', { count: 'exact', head: true })
            .eq('local_id', profile.local_id);

        // 4. Count local images
    ...
    const fetchLocalFamilies = async () => {
        if (!profile?.local_id) return;

        setLoadingFamilies(true);
        try {
            // Get species linked to this project with their families
            ...
            if (data) {
                // Count families and remove duplicates
            ...
            console.error('Error fetching local families:', error);
        } finally {
    ...
    // Scenario B: Local Admin (Gestor de Acervo) - Project Dashboard
    if (isLocalAdmin) {
        return (
            <div className="space-y-6 animate-fade-in-up">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <NavLink
                            to="/species"
                            className="flex items-center gap-4 p-4 bg-emerald-50 border border-emerald-100 rounded-xl hover:shadow-md transition-all group"
                        >
